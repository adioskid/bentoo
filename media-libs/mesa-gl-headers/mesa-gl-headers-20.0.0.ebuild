...
# for x in $(cat /var/db/pkg/media-libs/mesa-gl-headers-19.3.3/CONTENTS | cut -f2 -d" " | grep .h$); do 
...
# done
# cd /var/tmp/mhl; tar -cvJf /var/cache/portage/distfiles/${P}.tar.xz .
# rm -rf /var/tmp/mhl/

src_install() {
	dodir /usr/include
	(cd ${D}/usr/include && tar xvf ${DISTDIR}/${P}.tar.xz) || die "extract failed"
}