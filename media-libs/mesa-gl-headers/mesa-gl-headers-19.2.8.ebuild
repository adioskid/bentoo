...
# A quick way to generate a new headers archive based on the contents of the previous archive:
 for x in $(cat /var/db/pkg/media-libs/mesa-gl-headers-19.2.7/CONTENTS | cut -f2 -d" " | grep .h$); do 
 ...
 cd /var/tmp/mhl; tar cvf /var/cache/portage/distfiles/mesa-gl-headers-${PVR}.tar .
 xz /var/cache/portage/distfiles/mesa-gl-headers-${PVR}.tar
 rm -rf /var/tmp/mhl/

src_install() {
	dodir /usr/include
	(cd ${D}/usr/include && tar xvf /var/cache/portage/distfiles/mesa-gl-headers-${PVR}.tar.xz) || die "extract failed"
}
